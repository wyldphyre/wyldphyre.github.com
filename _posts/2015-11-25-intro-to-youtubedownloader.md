...
This is is my most recent Python scripting adventure and it came together quicker than I thought it would once I got started. If it sounds like something you'd like to use, you can find it [on Github](https://github.com/wyldphyre/YouTubeDownloader).

Here's the story of why it came about, and how I have put it to use in order to solve a long standing annoyance.
...
This desire was a combination of several things that came together to become a constant minor annoyance.
...
I could get around this to some degree my watching on my phone using mobile data, or my iPad tethered to my iPhone, but that solution was limited. 1.5 GB of mobile data per month doesn't go very far when watching video. Plus, ideally, I'd like to be able to watch the videos on the larger screen of my TV.

## Dashed Hopes  (ᵕ̣̣̣̣̣̣﹏ᵕ̣̣̣̣̣̣)

There was a period of time several years back when YouTube provided a way to get an RSS feed of a YouTube channel. It wasn't easy, but it could be done. And I'm a big fan of RSS. Great! That would be part of the solution to getting automation rolling.

But no. With an RSS feed you could add it to your feed reader of choice and get prompt notification of when new videos were available, but that didn't solve the downloading side of things. I'm sure there were tools around that could download the videos but I never found a good one while I was looking. And it's kind of a moot point these days since as far as I can tell it's been practically impossible to get an RSS feed out of YouTube for the last couple of years. I guess they really want you to watch videos on the website or in one of their apps.

There are also a plethora of websites out there that offer to download YouTube videos for you. You provide the URL of the video and after a little while they give you a link to download the video. I've tried a few here and there, but they tended not to be very good, and they were all a manual process that didn't get me any closer to an automated system.
...
- Trigger the workflow to save the URL to Dropbox. The URLs are saved into a folder with a time stamp for the name and '.queue' as the extension.
...
So I have Hazel monitor the Dropbox folder into which the files containing the URLs are saved. When Hazel detects a new file in the folder it triggers my YouTubeDownloader script.
...
- For each file, get the URL out of the file and pass it to the `youtube-dl` code, which will download it to another hard coded folder in my `~/Movies` folder.
...
At this point I have a folder, `~/Movies/YouTubeDownloads/`, that contains one or more videos. That's nice, but they aren't in their final resting place. They still need to make it to another computer where I store all my media.

To achieve that I used Hazel once again. I added the download folder into Hazel, and told it to move any video files it finds to a new location in my media store. Since the video file name contains the YouTube account name, I can use that to determine which folder to move certain file to, allowing me to categorise YouTube channels into different folder if I want to.
